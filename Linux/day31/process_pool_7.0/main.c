        ...
        for(i=0;i<ready_fd_num;i++)
        {
            ...
            if(evs[i].data.fd==socket_fd)
            {
                ...
                for(j=0;j<process_num;j++)//找到非忙碌的子进程
                {
                    ...
                    {
                        send_fd(p_manage[j].fd,new_fd,0);//发任务给子进程
                        p_manage[j].busy=1;//对应子进程被标记为忙碌  
            ...
            {
                if(evs[i].data.fd==p_manage[j].fd)//说明对应子进程发通知了
                {
