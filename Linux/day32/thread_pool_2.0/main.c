    ...
    while(1)
    {
        ...
        for(i=0;i<ready_fd_num;i++)
        {
            if(evs[i].data.fd==socket_fd)
            {
                ...
                pnew=(pNode_t)calloc(1,sizeof(Node_t));//先申请空间再上锁
                pnew->new_fd=new_fd;
                pthread_mutex_lock(&pq->mutex);//加锁
                que_set(pq,pnew);//放队列
                pthread_mutex_unlock(&pq->mutex);//解锁
                pthread_cond_signal(&f.cond);//使条件成立，唤醒一个子线程
            }
            if(evs[i].data.fd==exit_fds[0])//线程池要退出
            {
                ...
                for(j=0;j<thread_num;j++)//等待每一个子进程
                {
